...
# Note: I added an accumulator to this solution.
...
def generate_route(map, line, index, acc)
  value = acc + map[line].fetch(index).to_i
  if map.length-1 < line+1
    value
  else
    value = [generate_route(map, line+1, index, value), generate_route(map, line+1, index+1, value)].max
  end
end

#data = read_map("Problem18_source_sample.txt")
data = read_map("Problem18_source.txt")
puts generate_route(data, 0, 0, 0)
